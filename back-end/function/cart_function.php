...
{
    $sql="UPDATE carello  SET quantità=quantità+1, prezzo= prezzo+$prezzo WHERE id_utente=$utente AND id_prodotto=$prodotto ";
    $con->query($sql);
...
function decreaseProduct($prodotto, $prezzo, $utente, $con){
    $sql="UPDATE carello SET quantità=qunatità-1 , prezzo= prezzo=-1 WHERE id_utente=$utente AND id_prodotto=$prodotto ";
    $con->query($sql);
    $sql="UPDATE quantita SET Quantita=Quantita+1, WHERE id_Articolo=$prodotto ";
    $con->query($sql);
}
